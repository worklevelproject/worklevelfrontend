...
작성 기준일: 2026-09-11 (2026-09-12 worklevelbackend master 갱신분 반영 및 프론트 재연동 완료).
worklevel-src 프로토타입 화면을 기준으로, 이번 포팅에서 각 화면이 실제 백엔드 API로 동작하는지(✅),
브라우저에만 저장되는 목업인지(🧪), 아예 만들지 않았는지(⛔)를 정리한다. API가 있어도 화면이 원래
요구하던 것과 조금 다르게 단순화된 부분은 "차이" 칸에 적었다.
...
5인이상 판정 계산 로직 신설(단, 컨트롤러 없이 서버 내부에서만 계산 — 조회 API는 아직 없음).
**프론트도 이 변경분에 맞춰 재연동 완료** — 할 일 화면을 새 Task/TaskResponse API로 다시 짰고
(승인/반려 단계는 백엔드에서 없어졌음), 공지·인수인계·매장 정보 편집을 목업/읽기전용에서 실제 API
연동으로 바꿨다. 급여만 여전히 목업(백엔드 조회 API 자체가 없음).
...
| 화면 | 상태 | 연동 API | 차이 |
|---|---|---|---|
| 오늘 | ✅ | `GET .../owner/dashboard/attendance`, `GET .../owner/work-requests/attendance-corrections`, `GET .../owner/work-requests?status=REJECT`, `GET .../owner/employees/stats`, `GET .../notices`, `GET .../hand-overs` | 매출 카드만 🧪 목업(`lib/stores/mock.js`, 매출 도메인 없음). 공지·마감 노트 카드는 실제 API로 교체. 급여 예상은 실제 근무시간 + 목업 계산식 |
...
| 급여 | 🧪(부분) | `GET .../owner/employees/stats`의 실제 근무시간 사용 | 백엔드에 수당(Cost)·주휴수당 계산 로직은 생겼지만(체크아웃/정정 시 자동 계산) 이를 조회하는 컨트롤러가 아직 없어서, 여전히 프론트 계산식(`lib/utils/payroll.js`) 사용 — 조회 API 추가되면 교체 대상 |
...
| 내 급여 | 🧪(부분) | 내 accept된 work-request로 주간시간 직접 합산 | 계산식은 급여와 동일하게 목업. 백엔드 Cost 계산은 생겼지만 조회 API가 아직 없어 그대로(위 급여 행 참고) |
